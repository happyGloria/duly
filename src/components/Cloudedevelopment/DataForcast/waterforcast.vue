    ...
    <vue-scroll>
      <div class="contentoutbox">
        <div class="contentinbox">
          <!-- conten1 -->
          <div class="contentoutbox_contentbox1">水量短期预测</div>
          <!-- conten2 -->
          <div class="contentoutbox_contentbox2">
            ...
            <div class="contentoutbox_contentbox2_textbox">
              ...
              >水量短期预测是根据监测点的历史数据，预测未来一段时间的数据变化情况。 该算法实现的是短期数据预测，可以预测未来一天或几天内各个时间点的水量或压力数据。 主要应用于压力数据、流量数据和大用户数据的数据预测。</div>
            </div>
            <div class="contentoutbox_contentbox2_img">
              <img
                src="../../../assets/img/service_yuce.png"
                style="width:100%"
                alt
              >
            </div>
...
在进行预测之前，可以用window_wash_f对历史窗口进行清洗，有利于提高预测效果。
                </div>
            </div>
          </div>
          <!-- conten6 -->
...
如果某一时刻缺数据，须补充NaN 值。</td>
   ...
   {
          "Forecast_day" : 5  ,
          "History_window" : [7132.7,6913.4,6665.7,6347.3,6414.8,6366.6,6408.4,6409.4,6255.8,.....],
          "Point_count_1_day" : 288 
     }
...
一个时间步长。如下图：
          ...
          <div class="contentoutbox_contentbox9">
            ...
            <div class="contentoutbox_contentbox9_textbox">
              <textarea name id cols="100" rows="30" readonly>
    {"Data_out":[6384.5,6461.3,6433.2,6369.8,....]}
              </textarea>
            </div>
          </div>
          <!-- conten10 -->
          <div class="contentoutbox_contentbox10">
            ...
            <div class="contentoutbox_contentbox10_textbox">
              <textarea name id cols="100" rows="30" v-model="messagewaterforcast" placeholder></textarea>
            </div>
          ...
          <div class="contentoutbox_contentbox11">
            ...
            <div class="contentoutbox_contentbox11_textbox">
              <textarea name id cols="100" rows="30" v-model="resmsgwaterforcast" readonly></textarea>
            </div>
          </div>
          <!-- conten12 echarts部分 -->
...
import waterforcastecharts from "@/components/Cloudedevelopment/DataForcast/echarts/waterforcastecharts"
export default {
  name: "waterforcast",
  components:{
    waterforcastecharts
  ...
  data() {
    return {
      messagewaterforcast: JSON.stringify(defalftdata.waterforcastdata),
      resmsgwaterforcast: JSON.stringify(defalftdata.waterforcastresdata),
       messagewaterforcast2: {...defalftdata.waterforcastdata},
    };
  },
  mounted() {},
  methods: {
    Data_forecast() {
      var params = this.messagewaterforcast;
      this.$axios
        .post(Urlclass.UrlclassData_wash_f + "/Dataforecast/Data_forecast", params)
        .then(res => {
          var that = this;
          that.resmsgwaterforcast =JSON.stringify(res.data);
          var resdata=res.data
          var params2=that.messagewaterforcast2
          var washdata={params2,resdata}
          Bus.$emit("waterforcastfun", washdata);
        })
        .catch(err => {
          console.log("连接数据库失败！");
        });
