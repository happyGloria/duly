    ...
    <vue-scroll>
      <div class="contentoutbox">
        <div class="contentinbox">
          <!-- conten1 -->
          <div class="contentoutbox_contentbox1">监测数据噪声分析</div>
          <!-- conten2 -->
          <div class="contentoutbox_contentbox2">
            ...
            <div class="contentoutbox_contentbox2_textbox">
              ...
              >监测数据噪声分析是根据监测点一段时间的历史数据，计算各个时刻的数据噪声。数据噪声是监测数据稳定性的指标，噪声越小，表示监测数据越稳定，发生异常事件时越容易被发现。 主要应用于压力数据、流量数据和分区数据的噪声分析。</div>
            </div>
            <div class="contentoutbox_contentbox2_img">
              <img
                src="../../../assets/img/service_zaoshengqushi.png"
                style="width:100%"
                alt
              >
            </div>
            ...
            <div class="contentoutbox_contentbox6_textbox">
              <table class="contentoutbox_contentbox6_table">
                ...
                <tr>
                  ...
                  <td>输入历史窗口</td>
                  <td>N*1</td>
                  <td>须保证历史窗口内的数据在时间上连续，如果某一时刻缺数据，补充为NaN值。</td>
                </tr>
                <tr class="alt">
                  <td>Day_count</td>
                  <td>历史窗口数据天数</td>
                ...
                移动平均前后取数据点个数
                  ...
                  <td>1*1</td>
                  <td>建议取值： 30/时间步长， 即半个小时的时间点个数</td>
                </tr>
                <!-- <tr class="alt">
...
“Vary_day”: -2.4067157320535,
          ...
          <div class="contentoutbox_contentbox10">
            ...
            <div class="contentoutbox_contentbox10_textbox">
              <textarea name id cols="100" rows="30" v-model="messageobserdatazs" placeholder></textarea>
            </div>
          ...
          <div class="contentoutbox_contentbox11">
            ...
            <div class="contentoutbox_contentbox11_textbox">
              <textarea name id cols="100" rows="30" v-model="resmsgobserdatazs" readonly></textarea>
            </div>
          </div>
          <!-- conten12 echarts部分 -->
...
import obserdatazsecharts from "@/components/Cloudedevelopment/ObserData/echarts/obserdatazsecharts"
export default {
  name: "obserdatazs",
  components:{
    obserdatazsecharts
  ...
  data() {
    return {
      messageobserdatazs: JSON.stringify(defalftdata.obserdatazsdata),
      resmsgobserdatazs: JSON.stringify(defalftdata.obserdatazsresdata),
       messageobserdatazs2: {...defalftdata.obserdatazsdata},
    };
  },
  mounted() {},
  methods: {
    Trand_noise() {
      var params = this.messageobserdatazs;
      this.$axios
        .post(Urlclass.UrlclassData_wash_f + "/Trandnoise/Trand_noise", params)
        .then(res => {
          var that = this;
          that.resmsgobserdatazs =JSON.stringify(res.data);
          var resdata=res.data
          var params2=that.messageobserdatazs2
          var washdata={params2,resdata}
          Bus.$emit("obserdatazsfun", washdata);
        })
        .catch(err => {
          console.log("连接数据库失败！");
        });
