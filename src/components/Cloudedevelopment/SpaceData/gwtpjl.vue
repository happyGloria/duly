    ...
    <vue-scroll>
      <div class="contentoutbox">
        <div class="contentinbox">
          <!-- conten1 -->
          <div class="contentoutbox_contentbox1">管网拓扑聚类</div>
          <!-- conten2 -->
          <div class="contentoutbox_contentbox2">
            ...
            <div class="contentoutbox_contentbox2_textbox">
              ...
              >管网拓扑聚类算法是根据管网的节点坐标和拓扑信息，设定聚类个数，对管网节点进行聚类，将节点间拓扑距离相近的节点分为一类，输出聚类结果。 可用于在管网中寻找代表点，用一个节点代表一个类内的节点，减少模型分析计算量。</div>
            </div>
            ...
            <div class="contentoutbox_contentbox6_textbox">
              <table class="contentoutbox_contentbox6_table">
                ...
                <tr>
                  ...
                  <td>节点索引</td>
                  <td>N*1（ N 为节点个数）</td>
                  <td>节点索引,节点横坐标 ,节点纵坐标,三组信息需一一对应</td>
                </tr>
                <tr class="alt">
                  <td>Node_x</td>
                  <td>节点横坐标</td>
 ...
 -->
          ...
          <div class="contentoutbox_contentbox10">
            ...
            <div class="contentoutbox_contentbox10_textbox">
              <textarea name id cols="100" rows="30" v-model="messagegwtpjl" placeholder></textarea>
            </div>
          ...
          <div class="contentoutbox_contentbox11">
            ...
            <div class="contentoutbox_contentbox11_textbox">
              <textarea name id cols="100" rows="30" v-model="resmsggwtpjl" readonly></textarea>
            </div>
          </div>
          <!-- conten12 echarts部分 -->
...
/* import gwtpjlecharts from "@/components/Cloudedevelopment/ObserData/echarts/gwtpjlecharts" */
export default {
  name: "gwtpjl",
 /*  components:{
    gwtpjlecharts
  ...
  data() {
    return {
      messagegwtpjl: JSON.stringify(defalftdata.gwtpjldata),
      resmsggwtpjl: JSON.stringify(defalftdata.gwtpjlresdata),
       messagegwtpjl2: {...defalftdata.gwtpjldata},
    };
  },
  mounted() {},
  methods: {
    Sim_per_cal() {
      var params = this.messagegwtpjl;
      this.$axios
        .post(Urlclass.UrlclassData_wash_f + "/Simpercal/Sim_per_cal", params)
        .then(res => {
          var that = this;
          that.resmsggwtpjl =JSON.stringify(res.data);
          var resdata=res.data
          var params2=that.messagegwtpjl2
          var washdata={params2,resdata}
          Bus.$emit("gwtpjlfun", washdata);
        })
        .catch(err => {
          console.log("连接数据库失败！");
        });
