    ...
    <vue-scroll>
      <div class="contentoutbox">
        <div class="contentinbox">
          <!-- conten1 -->
          <div class="contentoutbox_contentbox1">管网距离聚类</div>
          <!-- conten2 -->
          <div class="contentoutbox_contentbox2">
            ...
            <div class="contentoutbox_contentbox2_textbox">
              ...
              >管网距离聚类是根据管网的节点坐标，设定聚类个数，对管网节点进行聚类，将节点间空间距离相近的节点分为一类，输出聚类结果。 可用于在管网中寻找代表点，用一个节点代表一个类内的节点，减少模型分析计算量。</div>
            </div>
           <!--  <div class="contentoutbox_contentbox2_img">
              <img
                src="../../../assets/img/service_tuopujulei.png"
                style="width:100%"
                alt
              >
            </div> -->
          ...
          <div class="contentoutbox_contentbox5">
            ...
            <div class="contentoutbox_contentbox5_textbox">
              <div class="contentoutbox_contentbox5_text">
                网址
                <a
                  href="http://112.64.170.158:9103/k_clustering.asmx/K_clustering"
                ></a>
              </div>
            </div>
          </div>
...
三组信息需一一对应</td>
    ...
    {
                "K" : 7 , 
                "X" :[ 54907.71 , 56538.6 ,55204.52 ,57098.03 , ...... ] , 
                "Y" :[ 13085.4 , 26050.8 ,25667.55,26102, ...... ] 
        }
          ...
          <div class="contentoutbox_contentbox9">
            ...
            <div class="contentoutbox_contentbox9_textbox">
              <textarea name id cols="100" rows="30" readonly>
  {"Center_num":[3873,8820,8055,2780,2783,1880,4887],"Class_num":[4,6,1,6,3,6,3,...]}	
 ...
 -->
          ...
          <div class="contentoutbox_contentbox10">
            ...
            <div class="contentoutbox_contentbox10_textbox">
              <textarea name id cols="100" rows="30" v-model="messagegwjljl" placeholder></textarea>
            </div>
          ...
          <div class="contentoutbox_contentbox11">
            ...
            <div class="contentoutbox_contentbox11_textbox">
              <textarea name id cols="100" rows="30" v-model="resmsggwjljl" readonly></textarea>
            </div>
          </div>
          <!-- conten12 echarts部分 -->
...
/* import gwjljlecharts from "@/components/Cloudedevelopment/ObserData/echarts/gwjljlecharts" */
export default {
  name: "gwjljl",
 /*  components:{
    gwjljlecharts
  ...
  data() {
    return {
      messagegwjljl: JSON.stringify(defalftdata.gwjljldata),
      resmsggwjljl: JSON.stringify(defalftdata.gwjljlresdata),
       messagegwjljl2: {...defalftdata.gwjljldata},
    };
  },
  mounted() {},
  methods: {
    Sim_per_cal() {
      var params = this.messagegwjljl;
      this.$axios
        .post(Urlclass.UrlclassData_wash_f + "/Simpercal/Sim_per_cal", params)
        .then(res => {
          var that = this;
          that.resmsggwjljl =JSON.stringify(res.data);
          var resdata=res.data
          var params2=that.messagegwjljl2
          var washdata={params2,resdata}
          Bus.$emit("gwjljlfun", washdata);
        })
        .catch(err => {
          console.log("连接数据库失败！");
        });
