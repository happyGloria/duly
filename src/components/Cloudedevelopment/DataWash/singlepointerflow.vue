<template>
  ...
  {"Data_out":224,"Err_code":0}
          ...
          <div class="contentoutbox_contentbox10">
            ...
            <div @click="Data_wash_f" class="contentoutbox_contentbox10_djyx">点击运行</div>
          </div>
          <!-- conten11 -->
          <div class="contentoutbox_contentbox11">
            ...
            <div class="contentoutbox_contentbox11_textbox">
              <textarea name id cols="100" rows="30" v-model="resmsg" readonly></textarea>
            </div>
          </div>
          <!-- conten12 echarts部分 -->
          <div class="contentoutbox_contentbox12">
            <singlepointerflowecharts/>
          </div>
          <!-- content结束 -->
        </div>
      </div>
...
import singlepointerflowecharts from "@/components/Cloudedevelopment/DataWash/echarts/singlepointerflowecharts"
export default {
  ...
  data() {
    return {
      message: JSON.stringify(defalftdata.singleflowdata),
      resmsg: JSON.stringify(defalftdata.singleflowresdata),
       message2: {...defalftdata.singleflowdata},
    };
  },
  mounted() {},
  methods: {
    Data_wash_f() {
      ...
      this.$axios
        .post(Urlclass.UrlclassData_wash_f + "/Datawashf/Data_wash_f", params)
        .then(res => {
          var that = this;
          that.resmsg =JSON.stringify(res.data);
          var resdata=res.data
          var params2=that.message2
          var washdata={params2,resdata}
          Bus.$emit("washdatafun", washdata);
        })
        .catch(err => {
          console.log("连接数据库失败！");
        });
