    ...
    <vue-scroll>
      <div class="contentoutbox">
        <div class="contentinbox">
          <!-- conten1 -->
          <div class="contentoutbox_contentbox1">单点大用户监测数据清洗</div>
          <!-- conten2 -->
          <div class="contentoutbox_contentbox2">
            ...
            <div class="contentoutbox_contentbox2_textbox">
              ...
              >根据大用户监测点的历史数据，判断某一个时刻的用水量是否正常。如出现由于监测设备故障导致的数据缺失或数据明显超出正常范围（如大用户用水量为负值）等异常情况，则根据历史数据规律，对该时刻的数据进行预测，如果数据正常，则不进行处理。同时输出数据的状态：正常、缺失或异常</div>
            </div>
            <div class="contentoutbox_contentbox2_img">
              <img
                src="../../../assets/img/service_dayonghuqingxi.png"
                style="width:100%"
                alt
              >
            </div>
          ...
          <div class="contentoutbox_contentbox3">
            <div class="contentoutbox_contentbox3_textbox">
              <div class="contentoutbox_contentbox3_text">单个时刻大用户监测数据清洗与历史窗口大用户监测数据清洗的对比见下表:</div>
            </div>
          </div>
          <!-- conten4 -->
          ...
          <div class="contentoutbox_contentbox5">
            ...
            <div class="contentoutbox_contentbox5_textbox">
              <div class="contentoutbox_contentbox5_text">
                网址
                <a
                  href="http://112.64.170.158:9100/api/Datawashuser/Data_wash_user"
                >http://112.64.170.158:9100/api/Datawashuser/Data_wash_user</a>
              </div>
            </div>
          </div>
          <!-- conten6 -->
  ...
  {"Data_out":224,"Err_code":0}
          ...
          <div class="contentoutbox_contentbox10">
            ...
            <div class="contentoutbox_contentbox10_textbox">
              <textarea name id cols="100" rows="30" v-model="messagebig" placeholder></textarea>
            </div>
            <div @click="Data_wash_user" class="contentoutbox_contentbox10_djyx">点击运行</div>
          </div>
          <!-- conten11 -->
          <div class="contentoutbox_contentbox11">
            ...
            <div class="contentoutbox_contentbox11_textbox">
              <textarea name id cols="100" rows="30" v-model="resmsgbig" readonly></textarea>
            </div>
          </div>
          <!-- conten12 echarts部分 -->
          <div class="contentoutbox_contentbox12">
            <singlebiguserecharts/>
          </div>
          <!-- content结束 -->
        </div>
      </div>
...
import singlebiguserecharts from "@/components/Cloudedevelopment/DataWash/echarts/singlebiguserecharts"
export default {
  name: "singlebiguser",
  components:{
    singlebiguserecharts
  ...
  data() {
    return {
      messagebig: JSON.stringify(defalftdata.singlebiguserdata),
      resmsgbig: JSON.stringify(defalftdata.singlebiguserresdata),
       messagebig2: {...defalftdata.singlebiguserdata},
    };
  },
  mounted() {},
  methods: {
    Data_wash_user() {
      var params = this.messagebig;
      this.$axios
        .post(Urlclass.UrlclassData_wash_f + "/Datawashuser/Data_wash_user", params)
        .then(res => {
          var that = this;
          that.resmsgbig =JSON.stringify(res.data);
          var resdata=res.data
          var params2=that.messagebig2
          var washdata={params2,resdata}
          Bus.$emit("washdatafunbig", washdata);
        })
        .catch(err => {
          console.log("连接数据库失败！");
        });
