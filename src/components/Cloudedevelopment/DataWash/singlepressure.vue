    ...
    <vue-scroll>
      <div class="contentoutbox">
        <div class="contentinbox">
          <!-- conten1 -->
          <div class="contentoutbox_contentbox1">单点压力监测数据清洗</div>
          <!-- conten2 -->
          <div class="contentoutbox_contentbox2">
            ...
            <div class="contentoutbox_contentbox2_textbox">
              ...
              >根据压力监测点的历史数据，判断某一个时刻的压力值是否正常。如出现由于监测设备故障导致的数据缺失或数据明显超出正常范围等异常情况（如压力数据出现负值），则根据历史数据规律，对该时刻的数据进行预测，如果数据正常，则不进行处理。同时输出数据的状态：正常、缺失或异常。</div>
            </div>
            <div class="contentoutbox_contentbox2_img">
              <img
                src="../../../assets/img/service_dandianyaliqingxi.png"
                style="width:100%"
                alt
              >
            </div>
          ...
          <div class="contentoutbox_contentbox3">
            <div class="contentoutbox_contentbox3_textbox">
              <div class="contentoutbox_contentbox3_text">单个时刻压力监测数据清洗与历史窗口压力监测数据清洗的对比见下表:</div>
            </div>
          </div>
          <!-- conten4 -->
          ...
          <div class="contentoutbox_contentbox5">
            ...
            <div class="contentoutbox_contentbox5_textbox">
              <div class="contentoutbox_contentbox5_text">
                网址
                <a
                  href="http://112.64.170.158:9100/api/Datawashp/Data_wash_P"
                >http://112.64.170.158:9100/api/Datawashp/Data_wash_P</a>
              </div>
            </div>
          </div>
          <!-- conten6 -->
...
最后一个数据的时间步长数
                  ...
                  <td>1*1</td>
                  <td>计算方法： (历史窗口最后一个数据的时间 – 要清洗数据的时间) / 时间步长</td>
                </tr>
                <!-- <tr class="alt">
                <td class="lefttd">Ernst Handel</td>
                <td>Roland Mendel</td>
  ...
  {"Data_out":224,"Err_code":0}
          ...
          <div class="contentoutbox_contentbox10">
            ...
            <div class="contentoutbox_contentbox10_textbox">
              <textarea name id cols="100" rows="30" v-model="messagep" placeholder></textarea>
            </div>
            <div @click="Data_wash_P" class="contentoutbox_contentbox10_djyx">点击运行</div>
          </div>
          <!-- conten11 -->
          <div class="contentoutbox_contentbox11">
            ...
            <div class="contentoutbox_contentbox11_textbox">
              <textarea name id cols="100" rows="30" v-model="resmsgp" readonly></textarea>
            </div>
          </div>
          <!-- conten12 echarts部分 -->
          <div class="contentoutbox_contentbox12">
            <singlepressureecharts/>
          </div>
          <!-- content结束 -->
        </div>
      </div>
...
import singlepressureecharts from "@/components/Cloudedevelopment/DataWash/echarts/singlepressureecharts"
export default {
  name: "singlepressure",
  components:{
    singlepressureecharts
  ...
  data() {
    return {
      messagep: JSON.stringify(defalftdata.singlepressuredata),
      resmsgp: JSON.stringify(defalftdata.singlepressureresdata),
     messagep2: {...defalftdata.singlepressuredata},
    };
  },
  mounted() {},
  methods: {
    Data_wash_P() {
      var params = this.messagep;
      this.$axios
        .post(Urlclass.UrlclassData_wash_f + "/Datawashp/Data_wash_P", params)
        .then(res => {
          var that = this;
          that.resmsgp =JSON.stringify(res.data);
         /*  console.log(
            "%cres返回的信息%c",
            "font-size:36px;font-weight:blod;color:red",
            "",
            res
          ); */
          var resdata=res.data
          var params2=that.messagep2
          ...
          Bus.$emit("washdatafunp", washdata);
        })
        .catch(err => {
          console.log("连接数据库失败！");
        });
